...
class CVRP:
    def __init__(self, M, D, nroV, capac, archivo, solI, opt, tADD, tDROP, tiempo, optimo):
        ...
        if self.__rank == 0:
            self.__mpiTxt = clsTxt("MPI-"+str(archivo))
    ...
    def tabuSearch(self):
        ...
        while(tiempoEjecuc < tiempoMax and porcentaje*100 > 5.0):
            ...
            if(time () - tCoord > self.__tiempoMPI):
                print ("Compartiendo    <<<<---------------------------------------------------------------- MPI")
                self.__rank = self.__comm.Get_rank()
                listaS = self.__comm.allgather((self.__S, self.__comm.Get_rank(), solucion_refer, Aristas, lista_tabu, nueva_solucion, ind_permitidos)) #, ind_permitidos, self.__rutas
                smCosto = listaS[0]
                for i in range(1,len(listaS)):
                    if(listaS[i][0].getCostoAsociado() < smCosto[0].getCostoAsociado()):
                        smCosto = listaS[i]
                self.__S = copy.deepcopy(smCosto[0])
                solucion_refer = copy.deepcopy(smCosto[2])
                Aristas = copy.deepcopy(smCosto[3])
                lista_tabu = copy.deepcopy(smCosto[4])
                nueva_solucion = copy.deepcopy(smCosto[5])
                ind_permitidos = copy.deepcopy(smCosto[6])
                # lista_tabu = copy.deepcopy(smCosto[2])
                # self.__rutas = copy.deepcopy(smCosto[4])
                # nuevas_rutas = copy.deepcopy(smCosto[4])
                # rutas_refer = copy.deepcopy(smCosto[4])
                # nueva_solucion = copy.deepcopy(smCosto[0])
                if self.__rank == 0:
